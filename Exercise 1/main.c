...
static MazeInfo generate_file(FILE *mazeFile) {
    ...
    //Read variables in first three lines. Note that the last variable in the first line is ignored, since we don't use the sensor.
    if (fscanf(mazeFile, "%ld %ld %*ld", &mi->maze->sizeY, &mi->maze->sizeX) != 2)
...
int main(int argc, char const *argv[])
{
    ...
    if (argc != 2) {
        fprintf(stderr, "Missing filename - Usage: ./out <file>\n");
        exit(-1);
    }
